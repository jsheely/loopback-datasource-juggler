...
function validateLength(attr, conf, err, options) {
  var value = getValue(this, attr);
  if (nullCheck.call(this, value, conf, err)) return;
...
function validateNumericality(attr, conf, err, options) {
  ...
  if (conf.int && value !== Math.round(value)) {
    return err('int');
  }
}
 ...
 */
function nullCheck(value, conf, err) {
  // First determine if value is defined
...
function formatErrors(errors, propertyValues) {
  ...
  return Object.getOwnPropertyNames(errors)
    ...
    .map(function(propertyName) {
      ...
      var propertyValue = getValue(propertyValues, propertyName);
